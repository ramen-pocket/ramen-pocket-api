...
export class StoreCommentGroupConstructor {
  ...
  private async buildStore(): Promise<Store> {
    const CLASS = StoreCommentGroupConstructor;
    const [rawStores, rawImages, rawBusinessHours, rawCourses] = (await Promise.all([
    ...
    ])) as [Schema.Store[], Schema.Image[], Schema.BusinessHour[], Schema.Course[]];
  ...
  public async load(): Promise<StoreCommentGroup> {
    const [store, records] = await Promise.all([this.buildStore(), this.buildRecords()]);
